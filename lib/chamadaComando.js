...
const grupo = require('../comandos/grupo'), utilidades = require('../comandos/utilidades'), diversao = require('../comandos/diversao'), admin = require('../comandos/admin'), rdono = require('../comandos/rdono'), info = require('../comandos/info'), figurinhas = require('../comandos/figurinhas'), maker = require('../comandos/maker'), downloads = require('../comandos/downloads')
...
module.exports = chamadaComando = async (client, message) => {
    try {
        ...
        const rownerNumber2 = process.env.NUMERO_RDONO2.trim()
        ...
        var comandoExiste = (
            ...
            lista_comandos.rdono.includes(command) ||
            lista_comandos.info.includes(command) ||
            lista_comandos.figurinhas.includes(command) ||
            lista_comandos.maker.includes(command) ||
            lista_comandos.downloads.includes(command)
        )
        const abrirMenu = commands.match(/comandos|comando|ajuda|menu|help/gmi) && !isGroupMsg && !comandoExiste
        ...
        if(comandoExiste){
            if(lista_comandos.utilidades.includes(command)){
                ...
                await queueMensagem.add(async ()=>{
                    //UTILIDADES
                    if(msgGuia) return await client.reply(from, guiaComandoMsg("utilidade", command), id)
                ...
                await queueMensagem.add(async ()=>{
                    //FIGURINHAS
                    if(msgGuia) return await client.reply(from, guiaComandoMsg("figurinhas", command), id)
                    await figurinhas(client,message)
                    consoleComando(isGroupMsg, "FIGURINHAS", command, "#ae45d1", t, username, formattedTitle)
                })
        }  else if(lista_comandos.maker.includes(command)){
                if(queueMensagemEspera) await client.sendText(from, criarTexto(msgs_texto.geral.fila_comando, queueMensagem.size))
                await queueMensagem.add(async ()=>{
                    //MAKER
                    if(msgGuia) return await client.reply(from, guiaComandoMsg("maker", command), id)
                    await maker(client,message)
                    consoleComando(isGroupMsg, "MAKER", command, "#ae45d1", t, username, formattedTitle)
                })
                ...
                await queueMensagem.add(async ()=>{
                    //DOWNLOADS
                    if(msgGuia) return await client.reply(from, guiaComandoMsg("downloads", command), id)
                ...
                await queueMensagem.add(async ()=>{
                    //GRUPO
                    if(msgGuia) return await client.reply(from, guiaComandoMsg("grupo", command), id)
                ...
                await queueMensagem.add(async ()=>{
                    //DIVERSÃO
                    if(msgGuia) return await client.reply(from, guiaComandoMsg("diversao", command), id)
                ...
                await queueMensagem.add(async ()=>{
                    //ADMIN
                    if(msgGuia) return await client.reply(from, guiaComandoMsg("admin", command), id)
                    await admin(client,message)
                    consoleComando(isGroupMsg, "DONO", command, "#d1d1d1", t, username, formattedTitle)
                }, {priority: 2})
             } else if(lista_comandos.rdono.includes(command)){
                if(queueMensagemEspera) await client.sendText(from, criarTexto(msgs_texto.geral.fila_comando, queueMensagem.size))
                await queueMensagem.add(async ()=>{
                    if(msgGuia) return await client.reply(from, guiaComandoMsg("rdono", command), id)
                    await rdono(client,message)
                    consoleComando(isGroupMsg, "REAL DONO", command, "#d1d1d1", t, username, formattedTitle)
                }, {priority: 2})
            } else if(lista_comandos.info.includes(command)){
                if(queueMensagemEspera) await client.sendText(from, criarTexto(msgs_texto.geral.fila_comando, queueMensagem.size))
                await queueMensagem.add(async ()=>{
                    //INFO
                    if(msgGuia) return await client.reply(from, guiaComandoMsg("info", command), id)
            ...
            if(isGroupMsg && (type == MessageTypes.IMAGE || type == MessageTypes.VIDEO) && grupoInfo.autosticker){
                ...
                if(botInfo().limitecomandos.status){
                    ...
                    if(limiteComando.comando_bloqueado) if(limiteComando.msg != undefined) return await client.reply(from, limiteComando.msg, id)
                }
                ...
                if(botInfo().limite_diario.status){
                    ...
                    else return await client.reply(from, criarTexto(msgs_texto.admin.limitediario.resposta_excedeu_limite, username, ownerNumber, rownerNumber, rownerNumber2), id)
                } else {
