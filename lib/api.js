...
module.exports = {
    ...
    obterNoticias : async ()=>{
        try {
            let res = await axios.get(`http://newsapi.org/v2/top-headlines?country=ar&apiKey=${process.env.API_NEWS_ORG.trim()}`), resposta = []
    ...
    obterClima: async local =>{
        try{
            ...
            const climaFotoURL = `http://es.wttr.in/${local}.png`
            const climaTextoURL = `http://es.wttr.in/${local}?format=Local%20=%20%l+\nClima actual%20=%20%C+%c+\nTemperatura%20=%20%t+\nUmedad%20=%20%h\nViento%20=%20%w\nLuna%20ahora%20=%20%m\nSale%20el%20Sol%20=%20%S\nPuesta%20de%20Sol%20=%20%s`
