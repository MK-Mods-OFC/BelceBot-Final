...
module.exports = {
    ...
    registrarRDono: async(id_usuario, nome)=>{
        var cadastro_usuario_rdono = {
            id_usuario,
            nome,
            comandos_total: 0,
            comandos_dia: 0,
        ...
        await db.usuarios.asyncInsert(cadastro_usuario_rdono)
    },

    verificarRDonoAtual : async(id_usuario)=>{
        let {limite_diario} = JSON.parse(fs.readFileSync(path.resolve("database/json/bot.json")))
        var usuario = await db.usuarios.asyncFindOne({id_usuario, tipo: "rdono"})
        if(!usuario){
            await db.usuarios.asyncUpdate({tipo: "rdono"}, {$set:{tipo: "bronze",  max_comandos_dia : limite_diario.limite_tipos.bronze}}, {multi: true})
            await db.usuarios.asyncUpdate({id_usuario}, {$set: {tipo : "rdono", max_comandos_dia: null}})
        }
    },

    registrarRDono2: async(id_usuario, nome)=>{
        var cadastro_usuario_rdono2 = {
            id_usuario,
            nome,
            comandos_total: 0,
            comandos_dia: 0,
        ...
        await db.usuarios.asyncInsert(cadastro_usuario_rdono2)
    },

    verificarRDono2Atual : async(id_usuario)=>{
        let {limite_diario} = JSON.parse(fs.readFileSync(path.resolve("database/json/bot.json")))
        var usuario = await db.usuarios.asyncFindOne({id_usuario, tipo: "rdono"})
        if(!usuario){
            await db.usuarios.asyncUpdate({tipo: "rdono"}, {$set:{tipo: "bronze",  max_comandos_dia : limite_diario.limite_tipos.bronze}}, {multi: true})
