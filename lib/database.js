...
module.exports = {
    ...
    verificarDonoAtual : async(id_usuario)=>{
        let {limite_diario} = JSON.parse(fs.readFileSync(path.resolve("database/json/bot.json")))
        var usuario = await db.usuarios.asyncFindOne({id_usuario, tipo: "creador"})
        if(!usuario){
            await db.usuarios.asyncUpdate({tipo: "creador"}, {$set:{tipo: "bronze",  max_comandos_dia : limite_diario.limite_tipos.bronze}}, {multi: true})
            await db.usuarios.asyncUpdate({id_usuario}, {$set: {tipo : "creador", max_comandos_dia: null}})
        }
    },
