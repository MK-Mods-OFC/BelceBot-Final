...
module.exports = {
    bloquearComandosGrupo: async (usuarioComandos, idGrupo)=>{
        ...
        var categorias = ['figurinhas', 'maker', 'utilidades', 'downloads', 'diversão'], primeiroComando = usuarioComandos[0]
        ...
        if(categorias.includes(primeiroComando)){
            ...
            switch(primeiroComando){
                case "figurinhas":
                    ...
                    break
                case "maker":
                    comandosCategoria = listaComandos.maker
                    break
                case "utilidades":
            ...
            for(var comando of usuarioComandos){
                if(listaComandos.utilidades.includes(comando) || listaComandos.diversao.includes(comando) || listaComandos.figurinhas.includes(comando) || listaComandos.maker.includes(comando) || listaComandos.downloads.includes(comando)){
                    if(grupoInfo.block_cmds.includes(comando)){
                        respostaBloqueio += criarTexto(msgs_texto.grupo.bcmd.resposta_variavel.ja_bloqueado, comando)
                    } else {
                        comandosBloqueados.push(comando)
                ...
                } else if (listaComandos.grupo.includes(comando) || listaComandos.admin.includes(comando) || listaComandos.rdono.includes(comando) || listaComandos.info.includes(comando)){
    ...
    desbloquearComandosGrupo: async (usuarioComandos, idGrupo)=>{
        ...
        var categorias = ['todos', 'figurinhas', 'maker', 'utilidades', 'downloads', 'diversão'], primeiroComando = usuarioComandos[0]
        if(categorias.includes(primeiroComando)){
            var comandosCategoria = []
            switch(primeiroComando){
                case "todos":
