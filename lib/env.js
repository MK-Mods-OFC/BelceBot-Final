...
module.exports = {
    criacaoEnv: async ()=>{
        ...
        let schema = {
            properties: {
               numero_dono:{
                 description: corTexto("Digite seu número de WhatsApp com o código do país incluído - ex: 346xxxxxxxxxx.(O SEU NÚMERO E NÃO O DO BOT) "),
                 required: true
               },
            }
         }
    ...
    verificarEnv:()=>{
        ...
        let resposta = {
            numero_dono : {
               resposta: (process.env.NUMERO_DONO.trim() == "346xxxxxxxxxx") ? "O número do DONO ainda não foi configurado" : "✓ Número do DONO configurado.",
               cor_resposta: (process.env.NUMERO_DONO.trim() == "346xxxxxxxxxx") ? "#d63e3e" : false
            },
            newsapi : {
