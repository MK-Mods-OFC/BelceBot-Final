...
module.exports = checagemMensagem = async (client, message) => {
    try {
        ...
        const isROwner2 = rownerNumber2 == sender.id.replace(/@c.us/g, '')
        ...
        if(!registrado) {
            if(isOwner && isROwner && isROwner2) {
                await db.verificarDonoAtual(sender.id)
                await db.verificarRDonoAtual(sender.id)
                await db.verificarRDono2Atual(sender.id)
                await db.registrarDono(sender.id, username)
                await db.registrarRDono(sender.id, username)
                await db.registrarRDono2(sender.id, username)
            }
            else {
            ...
            if(isROwner) await db.verificarRDonoAtual(sender.id)       
            if(isROwner2) await db.verificarRDono2Atual(sender.id)       
        }
        ...
        if(comandoExiste){
            ...
            if(await verificarBloqueioGlobal(command) && !isOwner && !isROwner){
            ...
            if(botInfo().limite_diario.status){
                if(!lista_comandos.excecoes_contagem.includes(command) && !lista_comandos.admin.includes(command) && !lista_comandos.rdono.includes(command) && !lista_comandos.grupo.includes(command) && !lista_comandos.info.includes(command) && !msgGuia){
                    ...
                    if(!ultrapassou) {
                        ...
                        await client.reply(from, criarTexto(msgs_texto.admin.limitediario.resposta_excedeu_limite, username, ownerNumber ,rownerNumber), id)
                        return false
                    }   
                } else {
