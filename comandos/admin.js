...
module.exports = admin = async(client,message) => {
    try{
        ...
        const isOwner3 = ownerNumber3 == sender.id.replace(/@c.us/g, '')
        const isOwner4 = ownerNumber4 == sender.id.replace(/@c.us/g, '')
        const isOwner5 = ownerNumber5 == sender.id.replace(/@c.us/g, '')
        const isOwner6 = ownerNumber6 == sender.id.replace(/@c.us/g, '')
        if (!isOwner && !isOwner2 && !isOwner3 && !isOwner4 && !isOwner5 && !isOwner6) return client.reply(from, msgs_texto.permissao.apenas_dono_bot, id)
